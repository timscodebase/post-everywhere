			...
			<div class="flex items-center justify-between p-6 border border-border rounded-xl bg-secondary/5">
				<div class="flex items-center gap-4">
					...
					<div>
						...
						{#if conn}
							<p class="text-xs text-primary font-bold">CONNECTED AS @{conn.platformUsername}</p>
						{:else}
				...
				{:else}
					<a href="/auth/login/{platform.id}" class="px-4 py-2 bg-primary text-primary-foreground text-xs font-bold rounded uppercase hover:opacity-90 transition-all">
						Connect
					</a>
				{/if}
			</div>
